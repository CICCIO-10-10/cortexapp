...
function _renderResult() {
  ...
  var pct = Math.max(0, Math.round(correct / (st.qs.length || 1) * 100));
  // 25/09/2026: errori totali (Inglese compreso) -> il ponte verso le flashcard
  // diventa il bottone PRINCIPALE, e la X chiede prima di uscire.
  var nWrong = stats.reduce(function (a, g) { return a + (g.wrong || 0); }, 0);
  st._nWrong = nWrong; st._resultShown = true;
...
// 25/09/2026: "Vuoi ripassare gli errori prima di uscire?" — pannello sopra il risultato.
function _askBeforeExit(ov, n) {
  try {
    var old = document.getElementById('tolc-exit-ask'); if (old) old.remove();
    var d = document.createElement('div');
    d.id = 'tolc-exit-ask';
    d.style.cssText = 'position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(5,5,10,.72);z-index:2147483001;padding:16px;';
    d.innerHTML =
...
document.addEventListener('click', function (e) {
  ...
  if (id === 'tolc-close') {
    if (_state && _state._resultShown && _state._nWrong > 0 && !_state._exitAsked) {
      ...
      try { track('tolc_exit_prompt_shown', { n: _state._nWrong }); } catch (e2) {}
